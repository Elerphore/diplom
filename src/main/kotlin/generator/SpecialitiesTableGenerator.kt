...
class SpecialitiesTableGenerator : TableTypeInterface {
    ...
    override fun generate() {
        ...
        months.forEachIndexed { index, month ->
            ...
            if(sheet == null) sheet = fact.createSheet("Специальности")
    ...
    private fun codeSpec(stds: List<Student>, diff: Int) {
        ...
        stds.groupByCodeSpec().forEach { (codeSpec, studscs) ->
            ...
            courses(studscs, previousGroupsAmount, diff)
    ...
    private fun courses(studgrps: List<Student>, previousGroupsAmount: Int, diff: Int) {
        var prevGRAmount = 0
        ...
        information(
            studgrps.groupByOsnova()["Бюджетная"] ?: emptyList(),
            false,
        ...
        information(
            studgrps.groupByOsnova()["Коммерческая"] ?: emptyList(),
            true,
            (previousGroupsAmount * 2) + (prevGRAmount * 2),
            diff
        )
    ...
    private fun courseName(previousGroupCount: Int, csName: String, diff: Int) {
        writeValueToCell(
            row = diff + 2,
